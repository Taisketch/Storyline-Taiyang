...
export const SymptomsSlide: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col items-center p-4">
      <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 serif">El Diagnóstico de Taiyang</h2>
      <p className="text-slate-500 dark:text-slate-400 mb-8">Explora los síntomas clave haciendo clic en los marcadores (+).</p>
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-12 w-full max-w-5xl">
        ...
        <div className="relative h-[500px] w-[300px] bg-white dark:bg-slate-800 rounded-3xl shadow-xl border-4 border-slate-200 dark:border-slate-700">
            {/* Silhouette abstract */}
            ...
            {hotspots.map((spot) => (
              <button
                ...
                className="absolute z-10 w-10 h-10 bg-red-600 rounded-full text-white flex items-center justify-center shadow-lg shadow-red-500/50 dark:shadow-red-900/50 hover:bg-red-500 hover:scale-110 transition-all animate-bounce ring-4 ring-red-100 dark:ring-red-900/30"
        ...
        {/* Context Panel */}
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg max-w-md border-l-4 border-slate-400 dark:border-slate-600 ring-1 ring-slate-200 dark:ring-white/5">
            <h3 className="text-lg font-bold mb-3 text-slate-900 dark:text-white uppercase tracking-wide">La Tríada de Diagnóstico</h3>
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                    <Activity size={18} className="text-red-600 dark:text-red-500" /> <span>Pulso Flotante</span>
                </div>
                <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                    <User size={18} className="text-red-600 dark:text-red-500" /> <span>Rigidez de Cabeza/Nuca</span>
                </div>
                 <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                    <Thermometer size={18} className="text-red-600 dark:text-red-500" /> <span>Aversión al Frío</span>
                </div>
            </div>
        </div>
      </div>
      ...
      <Modal isOpen={openPopup === 'skin'} onClose={() => setOpenPopup(null)} title="Aversión al Frío">
        ...
        <p className="text-sm bg-slate-100 dark:bg-slate-800 p-2 rounded italic border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">
